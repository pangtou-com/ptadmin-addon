...
class AddonServiceProvider extends ServiceProvider
{
    ...
    {
        $register = BootstrapManage::getServiceRegister();
        foreach ($register as $item) {
            $provider = $this->app->register($item);
            AddonManager::getInstance()->insert($provider);
            if (
                method_exists($provider, 'getExport')
                && method_exists($provider, 'getAddonName')
                && ($export = $provider->getExport())
                && ($addonName = $provider->getAddonName())) {
                AddonDirectivesManage::getInstance()->insert($addonName, $export);
            }
        }
    }
    ...
    protected function configureMiddleware(): void
    {
        ...
        $kernel->prependToMiddlewarePriority(AddonMiddleware::class);
    }
